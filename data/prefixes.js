...
const prefix = function (...args) {
  const adjustedLength = Math.max(args.length, 1); const names = args.slice(0, adjustedLength - 1); const
    data = args[adjustedLength - 1];
  return (() => {
    const result1 = [];
...
const add = function (...args) {
  const adjustedLength = Math.max(args.length, 1); const names = args.slice(0, adjustedLength - 1); const
    data = args[adjustedLength - 1];
  return Array.from(names).map((name) => (result[name].browsers = sort(result[name].browsers.concat(data.browsers))));
};
...
feature(gradients, { match: /a\sx/ }, (browsers) => {
  browsers = browsers.map((i) => { if (/op/.test(i)) { return i; } return `${i} old`; });
...
feature(flexbox, { match: /a\sx/ }, (browsers) => {
  browsers = browsers.map((i) => { if (/ie|firefox/.test(i)) { return i; } return `${i} 2009`; });
...
feature(require('caniuse-db/features-json/css-placeholder.json'), (browsers) => {
  browsers = browsers.map((i) => {
    ...
    if ((name === 'firefox') && (parseFloat(version) <= 18)) { return `${i} old`; } return i;
  });
