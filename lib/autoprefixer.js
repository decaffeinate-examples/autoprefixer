...
module.exports = postcss.plugin('autoprefixer', function(...reqs) {
  let options;
  ...
  if (options.browsers != null) { reqs = options.browsers; }
  ...
  const plugin = function(css, result) {
    ...
    if (options.remove !== false) { prefixes.processor.remove(css); }
    if (options.add !== false) { return prefixes.processor.add(css, result); }
  };
