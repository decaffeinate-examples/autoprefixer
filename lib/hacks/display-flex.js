...
class DisplayFlex extends Value {
  ...
  prefixed(prefix) {
    ...
    return prefix + (() => {
      if (spec === 2009) {
      if (this.name === 'flex') { return 'box'; } else { return 'inline-box'; }
    } else if (spec === 2012) {
      if (this.name === 'flex') { return 'flexbox'; } else { return 'inline-flexbox'; }
    } else if (spec ===  'final') {
      return this.name;
    }
    })();
  ...
  old(prefix) {
    ...
    if (prefixed) { return new OldValue(this.name, prefixed); }
  }
}
