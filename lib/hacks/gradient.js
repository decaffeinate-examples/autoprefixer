...
class Gradient extends Value {
  ...
  replaceFirst(params, ...words) {
    const prefix = words.map(function(i) {
      if (i === ' ') {
  ...
  newDirection(params) {
    ...
    for (let i = 2, end = params.length, asc = 2 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
  ...
  fixDirection(params) {
    ...
    return (() => {
      const result = [];
      for (let i = 0, end = params.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
        if (params[i].type === 'div') {
          break;
        }
        if (params[i].type === 'word') {
  ...
  fixRadial(params) {
    ...
    for (i = 4, end = params.length, asc = 4 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
      if (params[i].type === 'div') {
        break;
      } else {
  ...
  oldDirection(params) {
    ...
    if (params[0][0].value !== 'to') {
      ...
      let words = [];
      for (let node of Array.from(params[0].slice(2))) {
        if (node.type === 'word') {
  ...
  cloneDiv(params) {
    for (let i of Array.from(params)) {
      if ((i.type === 'div') && (i.value === ',')) {
        return i;
      }
    }
