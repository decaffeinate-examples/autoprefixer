...
class OldSelector {
  constructor(selector, prefix1) {
    ...
    this.prefixeds = [];
    for (const prefix of Array.from(selector.possible())) {
  ...
  isHack(rule) {
    ...
    while (index < rules.length) {
      ...
      let some = false;
      for (const [string, regexp] of Array.from(this.prefixeds)) {
  ...
  check(rule) {
    if (rule.selector.indexOf(this.prefixed) === -1) { return false; }
