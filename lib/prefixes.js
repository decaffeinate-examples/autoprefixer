...
class Prefixes {
  constructor(data, browsers, options) {
    ...
    [this.add, this.remove] = Array.from(this.preprocess(this.select(this.data)));
  ...
  select(list) {
    ...
    for (let name in list) {
      ...
      let notes = add.filter( i => i.note).map(i => {
        return this.browsers.prefix(i.browser) + ' ' + i.note;
      });
  ...
  preprocess(selected) {
    let name, prefixes, prop, props, value, values;
    const add = { selectors: [], '@supports': new Supports(Prefixes, this) };
    for (name in selected.add) {
      prefixes = selected.add[name];
      if ((name === '@keyframes') || (name === '@viewport')) {
        ...
        if (props) {
          ...
          for (prop of Array.from(props)) {
            if (!add[prop]) { add[prop] = { values: [] }; }
            add[prop].values.push(value);
          }
        } else {
          values = (add[name] != null ? add[name].values : undefined) || [];
      ...
      if (this.data[name].selector) {
        ...
        if (props) {
          ...
          for (prefix of Array.from(prefixes)) {
            ...
            if (name === 'align-self') {
              const a = add[name] != null ? add[name].prefixes : undefined;
              if (a) {
            ...
            for (prefixed of Array.from(olds)) {
  ...
  values(type, prop) {
    ...
    const global = data['*'] != null ? data['*'].values : undefined;
    const values = data[prop] != null ? data[prop].values : undefined;
  ...
  group(decl) {
    ...
    return {
      up(callback) { return checker(-1, callback); },
      down(callback) { return checker(+1, callback); }
    };
  }
}
