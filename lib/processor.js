...
class Processor {
  ...
  add(css, result) {
    ...
    css.walkRules(rule => {
      ...
      return Array.from(this.prefixes.add.selectors).map((selector) =>
    ...
    return css.walkDecls(decl => {
      ...
      for (let value of Array.from(this.prefixes.values('add', unprefixed))) {
  ...
  remove(css) {
    ...
    return css.walkDecls((decl, i) => {
      ...
      if (this.prefixes.remove[decl.prop] != null ? this.prefixes.remove[decl.prop].remove : undefined) {
        notHack = this.prefixes.group(decl).down(other => {
  ...
  displayType(decl) {
    for (let i of Array.from(decl.parent.nodes)) {
      if (i.prop === 'display') {
