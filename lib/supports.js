...
class Supports {
  ...
  prefixed(str) {
    ...
    for (let decl of Array.from(rule.nodes)) {
      for (let value of Array.from(this.prefixer().values('add', rule.first.prop))) {
        value.process(decl);
      }
  ...
  toRemove(str, all) {
    ...
    for (let checker of Array.from(this.all.cleaner().values('remove', unprefixed))) {
  ...
  convert(progress) {
    const result = [''];
    for (let i of Array.from(progress)) {
